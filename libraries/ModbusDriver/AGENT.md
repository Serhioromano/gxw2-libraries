...
- `mb_Timeout` is an `INT` output that reports the 0-based index (`iChannelID`) of the channel that timed out, held for one scan; it is `-1` when no channel has timed out. The watchdog block sets it via `MOV(fbTON2.Q, iChannelID, mb_Timeout)`, the per-scan default `mb_Timeout := -1` is set at the top of the POU, and the guard conditions use `mb_Timeout >= 0` / `mb_Timeout = -1` (not `> 0` / `= 0`).
- `iTimeOut` (per-channel, library-managed) is the consecutive-timeout counter. It is incremented in the watchdog block when a request times out and reset to `0` (`CHANNEL.iTimeOut := 0;`) on every successful `ADPRW` completion (both register and coil paths).
- `iChannelID` is the dedicated current-channel index; `iCount` is now only a generic loop counter (init-step device-zeroing loop). Step `1` resets `iChannelID` to `0` and reloads `CHANNEL := MB_CHANNELS[iChannelID]` so the bottom `MB_CHANNELS[iChannelID] := CHANNEL` store always targets the same channel it loaded. This fixes a bug where re-enabling the block mid-run copied a stale channel snapshot into `MB_CHANNELS[0]`.
